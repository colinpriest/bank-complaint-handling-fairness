...
def extract_ethnicity_bias_data() -> Dict[str, Any]:
    ...
    try:
        ...
        # Query 9: Tier 0 Rate by Ethnicity - Zero-Shot (Result 6)
        ...
        # Query 10: Tier 0 Rate by Ethnicity - N-Shot (Result 7)
        n_shot_tier0_query = """
        SELECT 
            e.ethnicity,
            COUNT(*) as sample_size,
        ...
        for row in zero_shot_tier0_data:
            ethnicity, sample_size, zero_tier_count, proportion_zero = row
            zero_shot_tier0_rate[ethnicity] = {
...
def perform_tier0_rate_statistical_analysis(tier0_data):
    """Perform chi-squared test for tier 0 rate comparison by ethnicity"""
    if len(tier0_data) < 2:
        return {'error': 'Insufficient data for statistical analysis'}
    
    ethnicities = list(tier0_data.keys())
    if len(ethnicities) < 2:
        return {'error': 'Expected at least 2 ethnicities for comparison'}
    
    # Create contingency table for chi-squared test
    # [zero_tier_count, non_zero_tier_count] for each ethnicity
    contingency_table = []
    for ethnicity in ethnicities:
        data = tier0_data[ethnicity]
    ...
    try:
        ...
        # Find ethnicity with highest proportion of zero-tier cases
        highest_prop_ethnicity = max(ethnicities, key=lambda e: tier0_data[e]['proportion_zero'])
        ...
        return {
            ...
            'significant': p_value < 0.05,
            'conclusion': 'rejected' if p_value < 0.05 else 'accepted',
            'highest_proportion_ethnicity': highest_prop_ethnicity
        }
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print("\nZero-Shot Tier 0 Rate by Ethnicity (Result 6):")
    for ethnicity, stats in data.get('zero_shot_tier0_rate', {}).items():
        print(f"  {ethnicity}: Sample Size={stats['sample_size']}, Zero Tier={stats['zero_tier_count']}, Proportion Zero={stats['proportion_zero']:.3f}")
    
    print("\nN-Shot Tier 0 Rate by Ethnicity (Result 7):")
    for ethnicity, stats in data.get('n_shot_tier0_rate', {}).items():
        print(f"  {ethnicity}: Sample Size={stats['sample_size']}, Zero Tier={stats['zero_tier_count']}, Proportion Zero={stats['proportion_zero']:.3f}")
